...
from sqlalchemy import ARRAY, Boolean, Column, DateTime, ForeignKey, Text, text
from sqlalchemy.dialects.postgresql import UUID
...
class author(Base):
    ...
    __table_args__ = {
        "schema": "public",
        "comment": "This is a duplicate of the authors table that will be used for the testing phase.",
    }
    ...
    unsubscribed = Column(Boolean, nullable=False, server_default=text("false"))


class twilio_message(Base):
    __tablename__ = "twilio_messages"
    __table_args__ = {
        "schema": "public",
        "comment": "This is a duplicate of twilio_messages, remember to switch back to real author and message tables",
    }

    id = Column(UUID, primary_key=True, server_default=text("gen_random_uuid()"))
    preview = Column(Text, nullable=False)
    type = Column(Text)
    delivered_at = Column(DateTime(True), nullable=False, index=True)
    updated_at = Column(DateTime(True))
    references = Column(ARRAY(Text()), nullable=False)
    created_at = Column(DateTime(True), nullable=False, server_default=text("now()"))
    external_id = Column(Text)
    attachments = Column(Text)
    from_field = Column(ForeignKey("public.authors.phone_number"), nullable=False, index=True)
    to_field = Column(ForeignKey("public.authors.phone_number"), nullable=False)
    is_broadcast_reply = Column(Boolean, nullable=False, index=True, server_default=text("false"))

    author = relationship("author", primaryjoin="twilio_message.from_field == author.phone_number")
    author1 = relationship("author", primaryjoin="twilio_message.to_field == author.phone_number")
